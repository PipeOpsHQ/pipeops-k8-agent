...
func TestValidateBaseURL(t *testing.T) {
	tests := []struct {
		...
		{"private IP 172.31.x", "http://172.31.255.255", true},
		{"link-local IP", "http://169.254.169.254", true},
...
func TestValidateRequestURL(t *testing.T) {
	tests := []struct {
		...
		{"empty URL", "", true},
		{"private IP", "http://192.168.1.1/api", true},
		{"metadata service AWS", "http://169.254.169.254/latest/meta-data", true},
